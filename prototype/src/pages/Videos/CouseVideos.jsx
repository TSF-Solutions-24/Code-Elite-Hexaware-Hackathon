...
const videos = [
  ...
  { title: 'Java Full Stack Setup and Installation', duration: '8 min', type: 'Video' },
  { title: 'Java Core Concepts', duration: '20 min', type: 'Video' },
  ...
  return (
    <section className="flex flex-col items-start max-w-full w-[94%]">
      ...
      {videos.slice(2).map((video, index) => (
        <Link to={'/videosplay'}><div className='w-full'><VideoItem key={index + 2} {...video} isFirst={false} /></div></Link>
      ))}
    </section>
  );
}
